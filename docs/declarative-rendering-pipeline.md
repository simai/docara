...
Основной portable publisher работает через декларативный конвейер. Старый
`PortableHtmlRenderer` сохранён побайтово только как ограниченный rollback.
Это не заявление о готовности всех компонентов или production-релиза.
...
View model сохраняет безопасное дерево с явным
`data-docara-navigation-depth`. Backend renderer рекурсивно подготавливает
зарегистрированные item templates; сами шаблоны не вычисляют active-state,
веса или уровни.

## Основная публикация

`PortableSiteBuilder` для каждой авторской и сгенерированной страницы:
...
4. сравнивает title, текст, regions, headings, links и Smart-семантику с
   доверенной проекцией;
...
7. публикует полный документ через зарегистрированный publisher template;
8. подключает общие immutable shell assets из `_docara`;
9. записывает hashes, provenance и publisher ID в
   `.docara/resolved-page-plans.json`.

Сейчас authoring-контракт включает `ui.alert` и `ui.button`. Если страница
содержит Smart-компонент вне принятого среза, основная сборка завершается
ошибкой. Destination заменяется только после полной успешной сборки отдельного
кандидатного каталога.

## Демонстратор макетов

Каталог `/examples/` не имеет отдельного rendering engine. Обычная скрытая
Markdown-страница проходит тот же loader, наследование, parser, compiler и
primary publisher. Строгий `examples/<id>.json` указывает на эту страницу и
перечисляет точные Markdown/JSON-источники, которые можно показать рядом с
живым результатом.

Projector создаёт только каталог и generic detail-поверхности. Он проверяет
schema, скрытый result route, collision, существование и размер источников,
запрещает symlink и выход из project root и записывает hashes в
`_docara/declarative-examples.json`. Поэтому демонстратор остаётся проверяемой
проекцией основного продукта, а не копией поведения.

Явный rollback:

```bash
DOCARA_PORTABLE_PUBLISHER=legacy php vendor/bin/docara build production
```

Rollback сохраняет URL и output inventory, но не является вторым режимом
авторинга и не расширяет поддержку компонентов.
...
Старый `PortableHtmlRenderer` нельзя удалять или переписывать без отдельной
цели вывода rollback. Тест фиксирует его принятый SHA-256 байт-в-байт.
