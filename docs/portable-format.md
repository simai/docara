...
Для `content/ru/guides/install.md` порядок фиксирован:

1. встроенные значения;
2. `docara.json`;
3. необязательный корневой `section.json`;
4. `content/ru/section.json`;
5. `content/ru/guides/section.json`;
6. `content/ru/guides/install.page.json`;
...
пустой объект. Структурный `layout.key` и полная карта `layout.regions`
являются исключением: после reset они восстанавливаются из зарегистрированного
layout contract.
...
- включённые `header`, `sidebar`, `main`, `outline`;
- выключенный пустой `footer`;
...
выбирает `docs`, русский locale в `content/ru` и `/ru/`, корневой redirect,
корневой `base_url`, бренд, широкий макет,
