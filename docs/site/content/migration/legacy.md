...
Одна portable-сборка содержит все локали из реестра `locales` и одну
`documentation_version`. Локали получают собственные Markdown-деревья и
URL-префиксы, явные fallback и переключатель языка. Отдельные версии по-прежнему
собираются под разными `base_url`. `socialImage`, edit/report links и общий variant
