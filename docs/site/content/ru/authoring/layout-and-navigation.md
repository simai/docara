...
Контейнер задаёт только максимальную ширину: на экране или внутри области уже
этого значения он автоматически занимает доступное место и не создаёт
горизонтальную прокрутку. Поэтому один и тот же `max-container-N` безопасно
работает в лендинге, в центральной колонке документации и на мобильном экране.
