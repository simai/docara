...
SIMAI Framework прогрессивно оформляет его как `sf-code-surface`: добавляет
заголовок с языком, доступную кнопку копирования и подсветку синтаксиса.
Номера строк не показываются по умолчанию и включаются только явным параметром
компонента. Копируется только точное содержимое `<code>`, без интерфейса.
Длинная строка прокручивается внутри одного code surface. Если runtime или
словарь неизвестного языка недоступен, исходный код остаётся безопасным
читаемым fallback.
