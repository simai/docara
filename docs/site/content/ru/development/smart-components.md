...
`docara.navigation` владеет данными дерева, раскрытием ветвей и product events,
а визуальные состояния получает из preset `documentation-tree` компонента
Menu SIMAI Framework. `docara.toc` владеет событием перехода по содержанию и
определяет активный заголовок при прокрутке; отступы, фокус и маркер активного
пункта принадлежат `sf-outline`, которому Docara передаёт
...
Размеры, отступы, цвета hover/active, радиусы и темы задаются компонентом и
токенами Framework. Поэтому светлая, тёмная и системная темы не требуют
отдельных шаблонов или системных CSS-переопределений Docara.

## Что уже принадлежит Framework

Docara компонует готовые универсальные поверхности:

- `sf-menu--documentation-tree` — многоуровневое меню документации;
- `sf-outline` — содержание страницы и активный маркер;
- `sf-code-surface` — заголовок кода, копирование и подсветка;
- `sf-kbd` — клавиши и сочетания в тексте и подсказках;
- `sf-modal` — поиск, мобильная навигация, мобильное содержание и настройки.

Product CSS допустим только для семантики самой Docara: раскладки её областей,
бренда и связей между компонентами. Он не должен исправлять внутренние классы,
состояния, тени, размеры контролов или focus Framework. Если универсального
поведения не хватает, его сначала добавляют и проверяют в SIMAI Framework, а
затем обновляют точный lock Docara.
