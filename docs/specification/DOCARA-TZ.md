...
Независимый `translation_tracking` формирует report относительно выбранной
`source_locale`, не меняет перечисленные правила публикации и не блокирует
сборку в режиме `report`. Совпадающие относительные пути связываются
автоматически, отличающиеся — только одинаковым явным `translation_key`.
Принятое состояние хранится в hash-bound `translations.lock.json`; обычная
сборка и status никогда не редактируют исходники.

Отдельный необязательный `documentation_tracking` сопоставляет страницы и
общие примеры базовой локали с публичным source contract проекта. Он не
дублирует source registry: `documentation.lock.json` содержит только принятые
связи и отпечатки, а status/build создают удаляемый отчёт. Принятие выполняется
только stale-safe планом; перевод и документационная актуальность остаются
...
на подтверждение. Вкладки доступны с клавиатуры. Высота блока следует за
активной вкладкой, а sandboxed HTML/CSS/JavaScript preview передаёт оболочке
свою фактическую высоту без `allow-same-origin` и ручного размера в контенте.
