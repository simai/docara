...
function renderTagTemplate(context) {
  ...
  const classes = ['sf-tag', `sf-tag--${context.type}`, `sf-tag--size-${context.size}`, 'inline-flex', 'flex-row', 'flex-nowrap', 'items-center', `${context.rootClass}`, context.type === 'checkbox' ? 'sf-tag--interactive' : '', context.closable ? 'sf-tag--removable' : '', context.active ? 'active' : '', context.disabled ? 'disabled' : ''].filter(Boolean);
