...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   renderIconButtonTemplate: () => (/* binding */ renderIconButtonTemplate)
/* harmony export */ });
/* harmony import */ var lit__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("c06304d43762");
/* harmony import */ var _default_css__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("fd800c850790");


function renderIconButtonTemplate(context) {
  const {
    size,
    type,
    scheme,
  ...
  const rootClasses = ['sf-icon-button', `sf-icon-button--size-${size || '1'}`, `sf-icon-button--${variant || 'icon'}`, `sf-icon-button--${type || 'default'}`, 'flex', 'items-cross-center', segment ? `segment-${segment}` : '', `sf-icon-button--${scheme || 'primary'}`, tightness ? `tightness-${tightness}` : '', radius ? `radius-${radius}` : '', rootClass, loading ? 'loading sf-icon-button-state-loading' : ''].filter(Boolean);
  const iconContent = context.component?.hasSlotContent?.('icon') ? context.component.getSlotContent('icon') : variant === 'close' ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
                            <sf-close root-class="pointer-event-none" size="${size || '1'}"></sf-close>` : (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
                            <sf-icon filled="${filled}" root-class="pointer-event-none" icon="${icon}" weight="400"></sf-icon>`;
  return (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
        <button
                ...
                aria-busy=${loading ? 'true' : lit__WEBPACK_IMPORTED_MODULE_0__.nothing}
        >
            ${iconContent}
        </button>
    `;
...
/* harmony import */ var _js_templates_default__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("adb62b9e4af2");
/* harmony import */ var _cl_classes_template_controlSpacing__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__("8fd843bdb9e7");



const COMPONENT_ATTRIBUTES = new Set(['template', 'variant', 'size', 'type', 'scheme', 'icon', 'filled', 'spacing', 'tightness', 'radius', 'segment', 'root-class', 'loading', 'disabled', 'native-type', 'aria-label', 'class', 'style', 'id', 'slot']);

function applyButtonAttributes(host, button) {
  host?.forwardHostAttributes(button, {
    exclude: COMPONENT_ATTRIBUTES,
    transferDataAttributes: true,
    storageKey: '__sfIconButtonForwardedDataAttributes',
    targetStorageKey: '__sfIconButtonAttributeKeys'
  });
}

class SfIconButton extends _cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__["default"] {
  constructor() {
    super();
  ...
  get scheme() {
    return this.getAttribute('scheme') || 'primary';
  }
  ...
  get tightness() {
    return (0,_cl_classes_template_controlSpacing__WEBPACK_IMPORTED_MODULE_2__.resolveControlSpacing)({
  ...
  get radius() {
    return this.getEnumAttr('radius', ['default', 'square', 'rounded'], '');
  }
  ...
  get loading() {
    return this.getBooleanAttr('loading', false);
  }
  ...
  get nativeType() {
    return this.getAttribute('native-type') || 'button';
  }
  ...
  templateContext() {
    ...
    return this.createTemplateContext({ ...props,
      ...(0,_cl_classes_template_controlSpacing__WEBPACK_IMPORTED_MODULE_2__.resolveControlSpacing)({
        spacing: this.getAttribute('spacing'),
        tightness: this.getAttribute('tightness')
      }),
      component: this
  ...
  template() {
    return (0,_js_templates_default__WEBPACK_IMPORTED_MODULE_1__.renderIconButtonTemplate)(this.templateContext());
  }

  resolveUpdateMode(changedAttributes = []) {
    const directPatchAttributes = ['disabled', 'aria-label', 'icon', 'loading'];
  ...
  updateDom(changedAttributes = []) {
    const button = this.querySelector('.sf-icon-button');
    ...
    if (changedAttributes.includes('icon')) {
      const iconNode = button.querySelector('.sf-icon');

      if (iconNode) {
        iconNode.textContent = this.icon;
      }
    }

    if (changedAttributes.includes('loading')) {
      button.classList.toggle('loading', this.loading);
      button.classList.toggle('sf-icon-button-state-loading', this.loading);
    ...
    applyButtonAttributes(this, button);
    return true;
  }
  ...
  afterRender() {
    ...
    if (!this._attributeObserver) {
      this._attributeObserver = this.observeForwardedHostAttributes(() => this.querySelector('.sf-icon-button'), {
        exclude: COMPONENT_ATTRIBUTES,
        transferDataAttributes: true,
        storageKey: '__sfIconButtonForwardedDataAttributes',
        targetStorageKey: '__sfIconButtonAttributeKeys'
      })._forwardHostAttributeObserver;
    }
  }
  ...
  onMouseEnter(handler, options) {
    if (typeof handler === 'function') {
      this.addEventListener('mouseenter', handler, options);
    }
  ...
  onMouseLeave(handler, options) {
    if (typeof handler === 'function') {
      this.addEventListener('mouseleave', handler, options);
    }

    return this;
  }

  setDisabled(nextValue = true) {
    if (nextValue) {
  ...
  onDisconnected() {
    this.disconnectForwardedHostAttributes();
    this._attributeObserver = null;
...
SfIconButton.define('sf-icon-button');
})();
