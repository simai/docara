...
function renderPageItem(item, context) {
  ...
  const pageLabel = item.type === "ellipsis" ? `Перейти к странице ${item.target}` : `Страница ${item.value}`;
  ...
  return (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
    <button
      type="button"
      class=${joinClasses("sf-page-number", "sf-page-number--square", "transition", item.selected ? "selected" : "")}
...
function renderPaginationTemplate(context) {
  ...
  const mainClasses = joinClasses("sf-pagination-main", "flex", "items-cross-center", "content-main-between", context.mainClass);
  const bottomClasses = joinClasses("sf-pagination-bottom", "flex", "items-cross-center", context.bottomClass);
  return (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
    <div class=${rootClasses}>
      ${context.top ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
      ...
      ${context.middle ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
            <div class=${mainClasses}>
              ...
              ${context.showPageSize ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
                    <div class="sf-pagination-count flex items-cross-center">
                      ...
                      >
                        ${(context.pageSizes || []).map(size => (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
                            <sf-list-item
                              type="text"
                              size="1"
                              text=${String(size)}
          ...
          ` : ""}
      ${context.bottom && context.showActions && context.actions?.length ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
            <div class=${bottomClasses}>
              ${context.showActions && context.actions?.length ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
                    <sf-dropdown
                      size="1"
                      type="outlined"
                      search="false"
                    ...
                    >
                      ${context.actions.map(action => (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
                          <sf-list-item
                            type="text"
                            size="1"
                            text=${action.text}
...
/* harmony import */ var _js_templates_default__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("3936dcf4bf6e");
...
function parsePageSizes(value, fallback = [10, 20, 30, 40]) {
  ...
  const parsed = String(value || "").split(",").map(item => Number(item.trim())).filter(Number.isFinite);
...
class SfPagination extends _cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__["default"] {
  static get props() {
    return {
      templateName: {
        attribute: "template",
  ...
  get showMoreText() {
    return this.getAttribute("show-more-text") || "Показать еще";
  }
  ...
  get showPageSize() {
    return this.getBooleanAttr("show-page-size", false);
  }
  ...
  get action() {
    ...
    return this.actions.find(item => item.selected && !item.disabled)?.value || this.actions.find(item => !item.disabled)?.value || "";
  }
  ...
  get current() {
    ...
    return Math.min(Math.max(parsed, 1), this.pageCount);
  }
  ...
  templateContext() {
    ...
    return this.createTemplateContext({ ...props,
      ...
      hasNext: this.current < this.pageCount,
  ...
  goToPage(page) {
    ...
    const next = Math.min(Math.max(parsed, 1), this.pageCount);
