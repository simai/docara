...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   renderProgressScaleTemplate: () => (/* binding */ renderProgressScaleTemplate)
/* harmony export */ });
/* harmony import */ var lit__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("c06304d43762");
...
function renderProgressScaleTemplate(context) {
  const size = context.size || '1';
  const value = normalizeProgressValue(context.value);
  const step = normalizeScaleStep(value);
  const rootClasses = ['sf-progress-scale', `sf-progress-scale--size-${size}`, 'flex', 'flex-col'];
  ...
  return (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
    <div class=${rootClasses.join(' ')} data-value=${String(value)}>
      <div class="sf-progress-scale-container grid grid-col-10 overflow-hidden">
        ${Array.from({
    length: 10
  }, () => (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`<div class="sf-progress-scale-block"></div>`)}
      </div>
      ${context.showText ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`<div class="sf-scale-text">${value}%</div>` : lit__WEBPACK_IMPORTED_MODULE_0__.nothing}
    </div>
  `;
}
...
/* harmony import */ var _js_templates_default__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("ac873709bae1");
...
function updateProgressScaleState(root, state = {}) {
  if (!root) return false;
  const value = normalizeProgressValue(state.value);
  const step = normalizeScaleStep(value);
  const showText = state.showText !== false;

  for (let currentStep = 0; currentStep <= 100; currentStep += 10) {
    root.classList.toggle(`sf-progress-scale--${currentStep}`, currentStep === step);
  }

  root.setAttribute('data-value', String(value));
  const text = root.querySelector('.sf-scale-text');

  if (text) {
    text.textContent = `${value}%`;
    text.classList.toggle('hidden', !showText);
  }

  return true;
}

class SfProgressScale extends _cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__["default"] {
  static get props() {
    return {
      templateName: {
        attribute: 'template',
  ...
  get showText() {
    return this.getBooleanAttr('show-text', true);
  }

  set showText(nextValue) {
    ...
    this.setAttribute('show-text', String(nextValue));
  }
  ...
  templateContext() {
    ...
    return this.createTemplateContext({ ...props,
      component: this,
      value: normalizeProgressValue(props.value)
    });
  }

  template() {
    return (0,_js_templates_default__WEBPACK_IMPORTED_MODULE_1__.renderProgressScaleTemplate)(this.templateContext());
  }

  get state() {
    return {
      size: this.size,
      value: normalizeProgressValue(this.value),
      showText: this.showText
    };
  }

  getProgress() {
    return normalizeProgressValue(this.value);
  }
  ...
  resolveUpdateMode(changedAttributes = []) {
    const directPatchAttributes = ['value', 'show-text'];
  ...
  updateDom() {
    const root = this.querySelector('.sf-progress-scale');
    if (!root) return false;
    return updateProgressScaleState(root, {
      value: this.value,
      showText: this.showText
    });
  }
...
SfProgressScale.define('sf-progress-scale');
})();
