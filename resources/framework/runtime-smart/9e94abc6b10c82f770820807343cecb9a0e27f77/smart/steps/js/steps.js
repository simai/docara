...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   renderStepsTemplate: () => (/* binding */ renderStepsTemplate)
/* harmony export */ });
/* harmony import */ var lit__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__("c06304d43762");
...
function renderStep(item, context) {
  const stepClasses = joinClasses('sf-step', 'transition', `sf-step--size-${context.size}`, `sf-step--stage-${item.stage}`, 'flex', 'items-center', 'justify-center', item.selected ? 'selected' : '', item.disabled ? 'disabled' : 'cursor-pointer');
  const iconName = item.stage === 'completed' ? 'check_circle' : item.stage === 'error' ? 'error' : '';
  return (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
    ...
    >
      ${iconName ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`<i class="sf-icon">${iconName}</i>` : ''}
      <div class="sf-step-container flex items-center justify-center">${item.label}</div>
    </div>
  `;
}

function renderStepsTemplate(context) {
  const rootClasses = joinClasses('sf-steps', `sf-steps--size-${context.size}`, 'flex', 'flex-col', context.rootClass);
  return (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
    <div class=${rootClasses}>
      <div class="sf-steps-container flex items-center">
        ${context.items.map((item, index) => (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`
            ...
            ${index < context.items.length - 1 ? (0,lit__WEBPACK_IMPORTED_MODULE_0__.html)`<i class="sf-icon">${context.separatorIcon}</i>` : ''}
          `)}
      </div>
    </div>
  `;
...
/* harmony import */ var _js_templates_default__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__("5723ee213bb1");
...
class SfSteps extends _cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__["default"] {
  static get props() {
    return {
      size: {
        default: '1'
  ...
  get current() {
    ...
    return Math.min(Math.max(this.getNumberAttr('current', 0), 0), maxIndex);
  }

  get errorStep() {
    const value = this.getAttribute('error-step');

    if (value === null || value === '') {
      return null;
    }

    const parsed = (0,_cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__.toNumber)(value, -1);
    return parsed >= 0 ? parsed : null;
  }

  get separatorIcon() {
    return this.getAttribute('separator-icon') || 'chevron_right';
  }

  get computedItems() {
    const errorStep = this.errorStep;
  ...
  template() {
    return (0,_js_templates_default__WEBPACK_IMPORTED_MODULE_1__.renderStepsTemplate)(this.templateContext());
  }
  ...
  goToStep(index) {
    const nextIndex = Math.min(Math.max((0,_cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__.toNumber)(index, this.current), 0), this.items.length - 1);
    const item = this.items[nextIndex];

    if (!item || item.disabled || nextIndex === this.current) {
      return this;
    }

    const previous = this.current;
    this.setAttribute('current', String(nextIndex));
    this.emitComponentEvent('step-change', {
      previous,
      current: nextIndex,
      item,
      items: this.items
  ...
  onStepChange(handler, options) {
    if (typeof handler === 'function') {
      this.addEventListener('sf-step-change', event => {
  ...
  handleRootClick = event => {
    ...
    const nextIndex = (0,_cl_classes_template_sfBaseElement__WEBPACK_IMPORTED_MODULE_0__.toNumber)(step.dataset.stepIndex, this.current);
