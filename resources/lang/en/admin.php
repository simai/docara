<?php

declare(strict_types=1);

return [
    'navigation' => ['pages' => 'Pages', 'menus' => 'Menus', 'site_settings' => 'Site Settings'],
    'pages' => [
        'title' => 'Pages',
        'eyebrow' => 'Content',
        'heading' => 'Pages',
        'description' => 'Create, review and publish the pages available on your Larena site.',
        'aria_label' => 'Pages',
    ],
    'framework_contract' => [
        'title' => 'Framework Explorer',
        'eyebrow' => 'Development / Framework',
        'heading' => 'Framework Explorer',
        'description' => 'Find a known Simai Framework building block, then inspect its source-backed context or a working example.',
        'explorer_eyebrow' => 'Pinned framework contract',
        'explorer_heading' => 'Building blocks',
        'explorer_description' => ':count records from the exact framework release. Details open only when needed.',
        'search_label' => 'Search',
        'search_placeholder' => 'Name, identifier or owner',
        'kind_label' => 'Kind',
        'kind_all' => 'All kinds',
        'reset' => 'Reset',
        'results' => 'Results: :count',
        'empty_filter' => 'No building blocks match these filters.',
        'columns' => ['item' => 'Building block', 'kind' => 'Kind', 'owner' => 'Owner', 'details' => 'Details'],
        'kinds' => ['utility' => 'Utility', 'component' => 'Component', 'smart-component' => 'Smart-component', 'recipe' => 'Recipe'],
        'details' => 'Details',
        'lifecycle' => 'Lifecycle',
        'readiness' => 'Readiness',
        'dependencies' => 'Requires',
        'source_references' => 'Source references',
        'demos_heading' => 'Try three real examples',
        'demos_description' => 'These examples prove the current Larena adapter. They do not imply that every registry record has a runnable Larena demo yet.',
        'button_demo_title' => 'Button',
        'button_demo_action' => 'Save draft',
        'layout_demo_title' => 'Layout utilities',
        'layout_demo_one' => 'Display',
        'layout_demo_two' => 'Flex direction and gap',
        'layout_demo_three' => 'Width and overflow',
        'table_demo_title' => 'Smart table',
        'table_demo_description' => 'Real read-only Docara page data, rendered through the resolved framework recipe.',
        'utilities_link' => 'Open Utility Explorer',
    ],
    'framework_utilities' => [
        'title' => 'Utility Explorer', 'eyebrow' => 'Development / Framework', 'heading' => 'Utility Explorer',
        'description' => ':count utility families from the pinned framework contract. Use a known recipe before inventing a class combination.',
        'pinned_contract' => 'Pinned framework contract', 'utility_heading' => 'Utilities',
        'utility_description' => 'The registry provides rule families and sources. It does not enumerate every allowed class value, so this screen never invents one.',
        'back_to_framework' => 'Back to Framework Explorer', 'search_label' => 'Search', 'search_placeholder' => 'Name, identifier, owner or readiness',
        'readiness_label' => 'Readiness', 'readiness_all' => 'All states', 'ready' => 'Ready', 'discoverable' => 'Discoverable', 'reset' => 'Reset',
        'results' => 'Results: :count', 'empty_filter' => 'No utilities match these filters.',
        'columns' => ['utility' => 'Utility', 'readiness' => 'Readiness', 'details' => 'Details'],
        'details' => 'Details', 'owner' => 'Owner', 'lifecycle' => 'Lifecycle', 'rule_families' => 'Rule families', 'asset_root' => 'Asset root',
        'not_declared' => 'Not declared by the registry', 'value_grammar_note' => 'Allowed class values are not enumerated by this registry. Follow the linked source family instead of guessing.',
        'requires' => 'Requires', 'source_references' => 'Source references',
        'recipes_heading' => ':count checked layout recipes', 'recipes_description' => 'Each example is a Larena composition of existing utility IDs. It is not a new framework primitive.',
        'used_utilities' => 'Uses:', 'demo_one' => 'First item', 'demo_two' => 'Second item', 'demo_three' => 'Third item', 'demo_title' => 'Page title', 'demo_action' => 'Action', 'demo_wide' => 'Wide content remains inside this scroll-safe region.',
    ],
    'actions' => [
        'create' => 'Create page',
        'create_first' => 'Create first page',
        'edit' => 'Edit',
        'preview' => 'Preview page',
        'back' => 'Back to pages',
        'back_to_edit' => 'Back to edit',
        'save' => 'Save page',
        'publish' => 'Publish page',
        'unpublish' => 'Unpublish page',
        'view_live' => 'View live page',
        'compose_blocks' => 'Compose blocks',
    ],
    'empty' => [
        'title' => 'No pages yet',
        'writer_text' => 'Create the first page to start building your site.',
        'reader_text' => 'No pages are available to review yet.',
    ],
    'columns' => [
        'page' => 'Page',
        'slug' => 'Slug',
        'locale' => 'Page language',
        'status' => 'Status',
        'action' => 'Action',
    ],
    'statuses' => [
        'draft' => 'Draft',
        'review' => 'In review',
        'published' => 'Published',
        'archived' => 'Archived',
        'denied' => 'Denied',
    ],
    'form' => [
        'edit_title' => 'Edit page',
        'create_title' => 'Create page',
        'eyebrow' => 'Content / Pages',
        'edit_heading' => 'Edit page',
        'create_heading' => 'Create page',
        'edit_description' => 'Update the page content and publication state.',
        'create_description' => 'Add a page with a clear title, URL slug and body.',
        'publication_status' => 'Publication status',
        'current_status' => 'Current status:',
        'publication_actions' => 'Publication actions',
        'unpublish_help' => 'Use “Unpublish page” to change publication state.',
        'no_hero_file' => 'No image',
    ],
    'fields' => [
        'hero_file' => 'Page image',
        'title' => 'Title',
        'slug' => 'Slug',
        'locale' => 'Page language',
        'body' => 'Body',
        'status' => 'Status',
    ],
    'preview' => [
        'title' => 'Preview: :title',
        'eyebrow' => 'Content / Pages / Preview',
        'description' => 'Protected preview of the saved page. This preview is not the public URL.',
        'status' => 'Protected preview · Current status:',
    ],
    'blocks' => [
        'title' => 'Page blocks: :title', 'eyebrow' => 'Content / Pages / Blocks', 'heading' => 'Compose “:title”',
        'description' => 'Build a saved draft from typed blocks. Public output changes only after publishing.',
        'read_only' => 'Your role can inspect the saved draft but cannot change or publish it.',
        'add_label' => 'Block type', 'add' => 'Add block', 'empty' => 'No blocks yet. The published Page keeps its legacy body until a composition is published.',
        'position' => 'Block :number', 'enabled' => 'Show this block', 'move_up' => 'Move block up', 'move_down' => 'Move block down', 'remove' => 'Remove block',
        'save_draft' => 'Save draft', 'saved' => 'Page block draft saved.',
        'publish_heading' => 'Publish the saved draft', 'publish_help' => 'Save changes first. Publishing validates the draft and replaces the public composition.', 'publish' => 'Publish page and blocks',
        'no_image' => 'No image',
        'types' => ['text' => 'Text', 'image' => 'Image', 'hero' => 'Hero', 'columns' => 'Two columns', 'cta' => 'Call to action'],
        'fields' => ['heading' => 'Heading', 'body' => 'Text', 'alignment' => 'Alignment', 'image' => 'Image from Files', 'alt' => 'Alternative text', 'caption' => 'Caption', 'eyebrow' => 'Eyebrow', 'title' => 'Title', 'cta_label' => 'Button label', 'cta_url' => 'Button URL', 'style' => 'Style', 'left_title' => 'Left title', 'left_body' => 'Left text', 'right_title' => 'Right title', 'right_body' => 'Right text'],
        'options' => ['left' => 'Left', 'center' => 'Center', 'default' => 'Default', 'accent' => 'Accent', 'primary' => 'Primary', 'secondary' => 'Secondary'],
        'validation' => ['invalid' => 'Check block types, required fields, order and URLs. Only relative, HTTPS and mailto links are allowed.', 'image' => 'Choose a currently available public image from Files.'],
    ],
    'messages' => [
        'created' => 'Page created.',
        'updated' => 'Page updated.',
        'published' => 'Page published.',
        'unpublished' => 'Page unpublished.',
    ],
    'menus' => [
        'title' => 'Menus', 'eyebrow' => 'Content / Menus', 'heading' => 'Site menus',
        'description' => 'Build ordered navigation from published Pages.', 'aria_label' => 'Site menus',
        'create_title' => 'Create menu', 'create_heading' => 'Create site menu', 'create_description' => 'Create one localized menu, then add published Pages.',
        'edit_description' => 'Code: :code · Locale: :locale', 'settings' => 'Menu settings',
        'items_heading' => 'Menu structure', 'items_help' => 'Change labels, numeric order and parent items. Lower order values appear first.',
        'empty_title' => 'No menus yet', 'empty_text' => 'Create the main menu to show published Pages in the public header.',
        'no_items' => 'This menu has no items.', 'add_item' => 'Add published Page', 'no_published_pages' => 'Publish a Page in this menu locale before adding an item.',
        'root' => 'Top level', 'active' => 'Active', 'inactive' => 'Inactive',
        'validation_heading' => 'Check the menu fields', 'delete_heading' => 'Delete menu',
        'delete_help' => 'The menu and its items disappear from the public header. Pages are not deleted.',
        'confirm_delete' => 'Delete this menu and all its items?', 'confirm_item_remove' => 'Remove this item from the menu?',
        'columns' => ['menu' => 'Menu', 'code' => 'Code', 'locale' => 'Locale', 'status' => 'Status', 'action' => 'Action'],
        'fields' => ['name' => 'Name', 'code' => 'Code', 'code_help' => 'Use main for the public header.', 'locale' => 'Locale', 'active' => 'Show publicly', 'page' => 'Published Page', 'label' => 'Navigation label', 'parent' => 'Parent item', 'order' => 'Order'],
        'actions' => ['create' => 'Create menu', 'edit' => 'Edit', 'view' => 'View', 'cancel' => 'Cancel', 'save' => 'Save settings', 'save_item' => 'Save item', 'add_item' => 'Add item', 'remove_item' => 'Remove item', 'delete' => 'Delete menu'],
        'messages' => ['created' => 'Menu created.', 'updated' => 'Menu updated.', 'deleted' => 'Menu deleted.', 'item_created' => 'Menu item added.', 'item_updated' => 'Menu item updated.', 'item_removed' => 'Menu item removed.', 'navigation_menu_exists' => 'A menu with this code and locale already exists.', 'navigation_page_unavailable' => 'Choose an available published Page in this locale.', 'navigation_parent_invalid' => 'Choose a parent from this menu.', 'navigation_cycle' => 'Menu nesting cannot contain a cycle.'],
    ],
    'site_settings' => [
        'title' => 'Site Settings', 'eyebrow' => 'Settings', 'heading' => 'Site Settings',
        'description' => 'Set the public site identity, branding, language and homepage.',
        'identity' => 'Site identity', 'branding' => 'Branding from Files', 'homepage' => 'Language and homepage',
        'read_only' => 'Your role can view these settings but cannot change them.',
        'validation_heading' => 'Check the site settings', 'no_image' => 'No image', 'no_page' => 'Not selected',
        'save' => 'Save site settings', 'saved' => 'Site settings saved.',
        'fields' => [
            'name_en' => 'Site name · English', 'name_ru' => 'Site name · Russian',
            'description_en' => 'Short description · English', 'description_ru' => 'Short description · Russian',
            'logo' => 'Logo', 'favicon' => 'Favicon', 'default_locale' => 'Default language',
            'homepage_en' => 'Homepage · English', 'homepage_ru' => 'Homepage · Russian',
        ],
        'validation' => [
            'image' => 'Choose a currently available public image from Files.',
            'page' => 'Choose a currently published public Page in this language.',
            'default_homepage' => 'Choose a published homepage for the default language.',
        ],
    ],
    'validation' => [
        'hero_file_invalid' => 'Choose a public image from Larena Files.',
        'title_required' => 'Enter a page title.',
        'title_string' => 'The page title must be text.',
        'title_max' => 'The page title may not exceed 255 characters.',
        'slug_required' => 'Enter a page slug.',
        'slug_string' => 'The page slug must be text.',
        'slug_max' => 'The page slug may not exceed 255 characters.',
        'slug_format' => 'The slug may contain lowercase letters, numbers and single hyphens only.',
        'slug_unique' => 'A page with this slug already exists.',
        'body_required' => 'Enter page content.',
        'body_string' => 'The page content must be text.',
        'status_required' => 'Choose a page status.',
        'status_invalid' => 'Choose an available page status.',
    ],
];
