<?php if ($view->items !== []) { ?>
<nav data-docara-breadcrumbs data-max-items="<?= $view->maxItems ?>" data-docara-breadcrumbs-ellipsis-label="<?= $view->expandLabel ?>" class="sf-breadcrumbs flex flex-nowrap items-cross-center min-w-0 overflow-x-auto overflow-y-hidden" aria-label="<?= $view->label ?>">
<?php foreach ($view->items as $index => $breadcrumb) { ?>
