...
<?php foreach ($group['fields'] as $field) { ?><fieldset class="docara-preferences-field flex flex-col gap-1 m-0 p-0 border-none" data-docara-preference-field="<?= $field['id'] ?>"><legend class="title-1 m-0 p-0"><?= $field['title'] ?></legend><?php if ($field['description'] !== '') { ?><p class="m-0 color-on-surface-variant"><?= $field['description'] ?></p><?php } ?>
