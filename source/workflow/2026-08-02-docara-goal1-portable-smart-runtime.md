...
| Batch | Result | Status |
| --- | --- | --- |
| G1.0 | exact baseline, Smart path map, accepted contract, launch/evidence contour | pass candidate |
| G1.1 | versioned source-pinned SF5 Smart v1 snapshot/adapter and fixtures | ready |
| G1.2 | descriptors/providers/compiler and ownership/path policy | pending |
| G1.3 | normalized invocation/context/strategy registry behind existing gateway | pending |
...
Current remaining after G1.0: 6 batches. Do not complete until all Goal 1 Done
When rows are evidenced or a stated stop condition is recorded. Next safe
batch: commit G1.0 separately, then continue directly to G1.1.
