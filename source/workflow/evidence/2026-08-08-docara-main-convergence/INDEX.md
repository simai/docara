...
Status: `main_convergence_accepted`
...
- final full PHPUnit on synchronized governance candidate `67d99b7…`: PASS,
  511 tests / 11,547 assertions. The earlier in-progress run had two
  ProjectContext assertions correctly catch an unsynchronized router edit;
  after canonical graph/handoff synchronization the complete ProjectContext
  suite passes 9 tests / 481 assertions and `project-context generate/check`
  returns `issues=[]`;
...
Formatting, JSON/context/diff and clean-status checks are repeated after this
governance-only result entry before `main` is advanced.
