...
class BuildCache
{
    ...
    public function setCachePath(string $path, string $outputRoot): void
    {
        ...
        $this->loadManifest();
        // Seed githubSha from manifest so cached SHA is respected even if fetch is skipped/disabled.
        if ($this->githubSha === null && isset($this->manifest['sha'])) {
            $this->githubSha = $this->manifest['sha'];
        }
    }
