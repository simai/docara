    ...
    class UniversalInlineParser implements InlineParserInterface
    {
        ...
        public function parse(InlineParserContext $inlineContext): bool
        {
            ...
            foreach ($this->registry->getSpecs() as $spec) {
                ...
                $attrStr = isset($matches['attrs'][0]) ? trim($matches['attrs'][0]) : '';
                $innerContent = isset($matches['inner'][0]) ? trim($matches['inner'][0]) : '';
                ...
                $node = new CustomTagInline(
                    $spec->type,
                    $attrs,
                    ['openMatch' => $matches, 'attrStr' => $attrStr, 'innerRaw' => $innerContent]
                );
