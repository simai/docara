...
final class PortableHtmlRenderer
{
    ...
    private function menu(array $nodes, int $depth = 1): string
    {
        ...
        foreach ($nodes as $node) {
            ...
            $elementClasses = 'sf-menu-element flex items-cross-center transition' . ($open ? ' open' : '');
            $current = ($node['active'] ?? false) === true ? ' aria-current="page"' : '';
            $activeClass = ($node['active'] ?? false) === true ? ' bg-surface-container weight-6' : '';
            ...
            if ($isBranch) {
                ...
                $disclosure = '<button type="button" data-docara-disclosure aria-expanded="' . $expanded
                    . '" aria-label="' . $this->escape($action . ': ' . $node['title'])
...
<script data-docara-shell-controller>(function(){
  ...
  function syncDisclosure(item){
    ...
    button.setAttribute('aria-label',(open?'Свернуть: ':'Развернуть: ')+title.trim());
  }
  function bindShell(){
