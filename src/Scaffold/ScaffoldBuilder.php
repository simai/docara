...
abstract class ScaffoldBuilder
{
    ...
    public function archiveExistingSite()
    {
        ...
        collect($this->allBaseFiles())->each(function ($file) use (&$directories) {
            ...
            if (in_array($file->getFilename(), self::PROTECTED_FILES, true)) {
                return;
            }
    ...
    public function restoreComposerDotJson()
    {
        ...
        if ($composer->count() && $docara_require = collect($composer->get('require'))->only(['simai/docara'])) {
            $this->writeComposer($composer->put('require', $docara_require));
        }
    }
