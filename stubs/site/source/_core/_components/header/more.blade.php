<div class="sf-more-wrap sf-float-wrap">
    ...
    <div class="sf-more-menu flex flex-col w-full p-1/3 gap-1/4 radius-default bg-surface-overlay bg-surface-1 dc-float-window w-max hidden">
        <div class="flex sf-more-menu_item">
            <button onclick="window.open(`{{$page->gitHubUrl()}}`, '_blank')" title="Изменить статью" class="sf-button flex text-start sf-button-settings sf-button--size-1 sf-button--outline sf-button--on-surface">
                <i class="sf-icon">edit</i>
                <span class="sf-button-text-container sf-button-text">{{$page->translate('edit article')}}</span>
            </button>
        </div>
        <div class="flex sf-more-menu_item">
            <button onclick="setIssue(`{{$page->github}}`)" title="Сообщить об ошибке" class="sf-button flex text-start sf-button-settings  sf-button--size-1 sf-button--outline sf-button--on-surface">
