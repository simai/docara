...
### Core methods
- `getType(): string` - tag type (e.g., `example`).
...
- `addClass(string $class): void` - append classes; whitespace-split, deduplicate, preserve order.
- `getMeta(): array` - parser metadata (e.g., `['openMatch' => ..., 'attrStr' => 'class:"mb-4"']`).

> Use `getMeta()['openMatch']` in a per-tag `attrsFilter($attrs, $meta)` or renderer to derive attributes from named regex captures.
...
- **Customization hooks**
  - `attrsFilter`: from `attrsFilter()`; signature `fn(array $attrs, array $meta): array`.
  - `renderer`: from `renderer()`; signature `fn(CustomTagNode $node, ChildNodeRendererInterface $children): mixed`.
...
- Node attributes merge correctly and `addClass()` deduplicates.
